...
export default {
    ...
    data(){
        return{
            ...
            messages:[],
            pseudo:'',
            socket : io('http://35.181.3.12:8080')
        }
    },
    methods:{
        joinServer(){
    ...
    async mounted(){
        ...
        // });
        const userId = 3;
        fetch(`http://35.181.3.12:8080/api/users/getPseudoById`,
        {
        ...
        fetch(`http://35.181.3.12:8080/api/intern/getOneGame/${paramsId}`)
        .then(response=>response.json())
        .then(data => {
        ...
        fetch(`http://35.181.3.12:8080/api/intern/getDiscussion/${paramsIdDiscussion}`)
        .then(response=>response.json())
        .then(data => {
            console.log('my data : ',data);
